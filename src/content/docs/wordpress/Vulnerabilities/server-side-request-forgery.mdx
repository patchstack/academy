...
head:
    - tag: title
      content: Learn about Server Side Request Forgery (SSRF)
contributors:
    - rafiem
---
...
<LinkCard
  title="Learn how to secure your code against Server Side Request Forgery"
  href="/academy/wordpress/securing-code/server-side-request-forgery/"
/>
...
To exploit this, any unauthenticated user needs to perform a POST request to the `/wp-admin/admin-ajax.php` endpoint specifying the needed parameter to trigger the `wp_remote_get` function.
