...
Sensitive data can be leaked through hooks or actions when used without consideration for the user permissions. Below is an example of vulnerable code:
...
If the logs are stored by the plugin without any .htaccess or randomization of the log filename, it might cause sensitive data leak.
