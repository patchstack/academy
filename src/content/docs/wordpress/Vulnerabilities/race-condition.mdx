...
To exploit this, any authenticated user just needs to perform two or more parallel POST request to the `/wp-admin/admin-ajax.php` endpoint specifying the needed parameters.

Note: This might not work on a single try. Trying to send more than two concurrent requests is recommended to increase the chance of success.
