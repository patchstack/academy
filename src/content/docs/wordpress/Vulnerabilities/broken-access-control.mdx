...
In the case when an action is only protected by nonce instead of permission check, there is a chance that the nonce might be leaked to the low-privileged users. It can lead to broken access control issues in such cases.
