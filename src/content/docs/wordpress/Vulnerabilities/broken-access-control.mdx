...
Insecure Direct Object Reference (IDOR) is one of the vulnerabilities which results due to broken access control. It involves changing any entity's ID to another which a user is not supposed to access.

For example, if there is a page that views orders by order id in the URL using the ?order_id=X parameter, if the user can just change the order ID to view another user's order details, it is called an IDOR.
