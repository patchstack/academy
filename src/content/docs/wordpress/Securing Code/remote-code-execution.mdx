...
This article covers cases of possible ways to secure the code from RCE vulnerability on WordPress. This includes applying a proper function to check for the user's input.
...
For a dynamic function call or function call using `call_user_func` function, we recommend using a whitelist check on the allowed functions that can be called.
